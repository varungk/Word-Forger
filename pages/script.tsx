...
const Blog = () => {
    const { user, getScriptData,getScriptTypes,getData,createScriptHistory,getScriptHistory} = useAuth();
    const [scriptType, setScriptType] = useState<string[]>([]);
    const [finalResult,setfinalResult] = useState<any>();
    const [loading,setLoading] = useState(false)
    const [historyData,setHistoryData] = useState<any>()
    const [historyTabClicked,setHistoryTabClicked] = useState(true)
    const [noOfgetScript,setNoOfgetScript] = useState(true)
    ...
    useEffect(() => {
        ...
        if(noOfgetScript){
            getScriptTypes().then((result: string[]) => {
                setScriptType(result);
                setNoOfgetScript(false)
            });
        }
        if(finalResult != null){
            setLoading(false)
        ...
        if(historyTabClicked){
            ...
            getScriptHistory(user.uid).then((result: any) => {
                if(result){
                    setHistoryData(result.history);
                }
            });
        }
    }, [getScriptTypes,noOfgetScript, user,setLoading,finalResult,getScriptHistory,historyData,historyTabClicked]);

    useEffect(() => {
        if (!selectedScriptType) return;

        getScriptData(selectedScriptType).then((result: any) => {
            setFormData(result);
        });
        console.log("gerr")
    ...
    }, [getScriptData, selectedScriptType,getData]);
    ...
    const onSubmit = async (values: any) => {
        ...
        if(data.result){
            ...
            createScriptHistory(hisData,user.uid)
